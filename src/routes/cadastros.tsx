...
type CatalogForm<T extends FieldValues, TOutput extends FieldValues | undefined = T> = UseFormReturn<T, unknown, TOutput>;
...
function TextField<T extends FieldValues, O extends FieldValues | undefined = T>({form,name,label,type="text"}:{form:CatalogForm<T,O>;name:FieldPath<T>;label:string;type?:string}){return <FormField control={form.control} name={name} render={({field})=><FormItem><FormLabel>{label}</FormLabel><FormControl>{type==="number"?<NumberField field={field}/>:<Input {...field} value={String(field.value??"")}/>}</FormControl><FormMessage/></FormItem>}/>}
function BoolField<T extends FieldValues, O extends FieldValues | undefined = T>({form,name,label}:{form:CatalogForm<T,O>;name:FieldPath<T>;label:string}){return <FormField control={form.control} name={name} render={({field})=><FormItem className="flex items-center justify-between rounded-xl border p-3"><FormLabel>{label}</FormLabel><FormControl><Switch checked={Boolean(field.value)} onCheckedChange={field.onChange}/></FormControl></FormItem>}/>}
function SelectFormField<T extends FieldValues, O extends FieldValues | undefined = T>({form,name,label,options}:{form:CatalogForm<T,O>;name:FieldPath<T>;label:string;options:{value:string;label:string}[]}){return <FormField control={form.control} name={name} render={({field})=><FormItem><FormLabel>{label}</FormLabel><SelectField value={String(field.value??"")} onChange={field.onChange} placeholder="Selecione" options={options}/><FormMessage/></FormItem>}/>}
function DateField({form}:{form:CatalogForm<TariffFormInput,TariffForm>}){return <FormField control={form.control} name="atualizado_em" render={({field})=><FormItem className="flex flex-col"><FormLabel>Atualizado em</FormLabel><Popover><PopoverTrigger asChild><FormControl><Button variant="outline" className={cn("justify-start text-left font-normal",!field.value&&"text-muted-foreground")}><CalendarIcon className="h-4 w-4"/>{field.value?format(field.value,"dd/MM/yyyy"):"Selecione"}</Button></FormControl></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={field.value} onSelect={field.onChange} initialFocus className="p-3 pointer-events-auto"/></PopoverContent></Popover><FormMessage/></FormItem>}/>}
