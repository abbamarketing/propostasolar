...
function CompanyForm({ company, companyId }: { company: any; companyId?: string | null }) {
  ...
  return <Card><CardHeader><CardTitle className="flex items-center gap-2"><Building2 /> Empresa</CardTitle></CardHeader><CardContent className="space-y-6"><div className="flex flex-wrap items-center gap-4"><div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-xl border bg-surface">{form.logo_url ? <img src={form.logo_url} alt="Logo da empresa" className="h-full w-full object-contain" /> : <Camera className="text-muted-foreground" />}</div><Input type="file" accept="image/*" onChange={(e) => uploadLogo(e.target.files?.[0])} className="max-w-xs" /></div><div className="grid gap-4 md:grid-cols-2"><Field label="Razão social" value={form.razao_social} onChange={(v) => setForm({ ...form, razao_social: v })} /><Field label="Nome fantasia" value={form.nome_fantasia} onChange={(v) => setForm({ ...form, nome_fantasia: v })} /><Masked label="CNPJ" as={CnpjInput} value={form.cnpj} onChange={(v) => setForm({ ...form, cnpj: v })} /><Field label="IE" value={form.inscricao_estadual} onChange={(v) => setForm({ ...form, inscricao_estadual: v })} /><Masked label="CEP" as={CepInput} value={form.endereco_cep} onChange={(v) => setForm({ ...form, endereco_cep: v })} /><Field label="Cidade" value={form.endereco_cidade} onChange={(v) => setForm({ ...form, endereco_cidade: v })} /><Field label="UF" value={form.endereco_uf} onChange={(v) => setForm({ ...form, endereco_uf: v })} /><Field label="Logradouro" value={form.endereco_logradouro} onChange={(v) => setForm({ ...form, endereco_logradouro: v })} /><Field label="E-mail" value={form.email} onChange={(v) => setForm({ ...form, email: v })} /><Masked label="Telefone" as={PhoneInput} value={form.telefone} onChange={(v) => setForm({ ...form, telefone: v })} /><Masked label="WhatsApp" as={PhoneInput} value={form.whatsapp} onChange={(v) => setForm({ ...form, whatsapp: v })} /><Field label="Site" value={form.site} onChange={(v) => setForm({ ...form, site: v })} /><Color label="Cor primária" value={form.cor_primaria} onChange={(v) => setForm({ ...form, cor_primaria: v })} /><Color label="Cor secundária" value={form.cor_secundaria} onChange={(v) => setForm({ ...form, cor_secundaria: v })} /><Field label="Responsável técnico" value={form.responsavel_tecnico_nome} onChange={(v) => setForm({ ...form, responsavel_tecnico_nome: v })} /><Field label="CREA" value={form.responsavel_tecnico_crea} onChange={(v) => setForm({ ...form, responsavel_tecnico_crea: v })} /><Field label="E-mail técnico" value={form.responsavel_tecnico_email} onChange={(v) => setForm({ ...form, responsavel_tecnico_email: v })} /></div><Button onClick={() => save.mutate()} disabled={save.isPending}><Save /> Salvar empresa</Button></CardContent></Card>;
}
...
function ProfileForm({ profile }: { profile: any }) {
  ...
  return <Card><CardHeader><CardTitle>Meu perfil</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2"><Field label="Nome" value={form.nome} onChange={(v) => setForm({ ...form, nome: v })} /><Masked label="Telefone" as={PhoneInput} value={form.telefone} onChange={(v) => setForm({ ...form, telefone: v })} /><Field label="Cargo" value={form.cargo} onChange={(v) => setForm({ ...form, cargo: v })} /><Field label="Avatar URL" value={form.avatar_url} onChange={(v) => setForm({ ...form, avatar_url: v })} /><div className="md:col-span-2"><Button onClick={() => save.mutate()}><Save /> Salvar perfil</Button></div></CardContent></Card>;
}
